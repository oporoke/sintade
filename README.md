...
### In Progress
- M2 — Identity (Days 11–20, weeks 3–4): in progress — full MVP backend plus Angular UI done (Days 11–17): domain types, register, login, refresh rotation + reuse detection, logout, `GET /me`, verify-email, forgot/reset-password, rate limits, CSRF double-submit, security headers, and Angular signup/login/verify/reset screens with silent refresh and a route guard, proven end to end by a real Playwright e2e test; `tenancy` crate, `WorkspaceContext`, `Permission`/`authorize()`, and the generated tenant-isolation test harness done (Day 18); profile settings, logout everywhere and OpenAPI-generated DTOs done (Day 19); buffer + staging demo next (Day 20).
