...
| Segment | Primary job | Example use |
| --- | --- | --- |
| Individual professionals | Replace a meeting with a video | Bug report, code walkthrough, status update |
| Teams (5–200 seats) | Async communication in a shared library | Onboarding, design review, support replies |
| Educators and trainers | Lessons and tutorials | Course clips with captions |
| Support and sales | Personalised video replies | Customer walkthroughs with a CTA |

**First segment: developers and tech teams** — bug reports, code walkthroughs, PR explanations and async standups. Other segments follow once this one retains. Plans are priced in TZS for the local market.
...
- Native desktop or mobile recording apps (browser-only capture; plus a companion browser extension built alongside the MVP).
- Live streaming or real-time video calls.
- A full timeline video editor (trim, cut and overlays only).
- Recording on mobile browsers (not supported by the platform APIs); mobile is view-only.
...
| 9 | Sharing and access | Unguessable slug; private / link / public visibility | Workspace-only; password; expiry; invite by email; download toggle; revoke/regenerate; Open Graph + oEmbed | Signed expiring segment URLs; domain-restricted embeds; viewer watermark |
