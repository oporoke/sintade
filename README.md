...
![Status](https://img.shields.io/badge/status-in--development%20(Day%2010%2C%20M1%20foundation%20complete)-yellow)
...
> [!IMPORTANT]
> **Repository status: in development (Day 10 of the [daily build plan](docs/plan/daily-build-plan.md) — M1 Foundation milestone complete).** The Cargo workspace (`crates/kernel` — renamed from `core`, see [ADR-0003](docs/adr/0003-rename-core-crate-to-kernel.md) —, `crates/platform`, `bin/api`, `bin/worker`) exists and builds; local dependencies (Postgres, MinIO, Mailpit) run via `compose.yml` + `just deps-up`; the API serves `/healthz` and `/readyz` (with a real DB check) on `:8080`, with request-ID/trace/CORS/body-limit/timeout middleware and RFC 9457 problem+json error responses; `platform::ObjectStore` presigns direct-to-MinIO PUT/GET URLs; `bin/worker` runs a real `SKIP LOCKED` job queue with exponential-backoff retries and a dead-letter queue, a transactional outbox relay (LISTEN/NOTIFY + polling fallback) delivering domain events to in-process subscribers exactly once, and a `SendEmail` job sending real mail via SMTP (Mailpit locally); an Angular shell (`web/`) exists with routing, an `ApiClient`, an error interceptor, theme tokens and a `CapabilityService` shown on a `/debug` page, verified cross-browser (Chrome, Firefox, WebKit) in CI; GitHub Actions CI (fmt, clippy, sqlx check, tests, lint, Angular build, e2e, audit) is green and builds/pushes Docker images for api/worker/web on merge to `main`. **No staging VPS exists yet** — the auto-deploy job is fully wired but gated on secrets that aren't configured (see `docs/plan/PROGRESS.md`). No feature-module business logic exists yet. This README is derived entirely from the *Sintade — Product & Architecture Document* (the design document). Every feature, endpoint, table, variable and procedure below is **planned**, not implemented, unless explicitly marked otherwise. Items that the design document does not settle are marked `TODO: Verify`.
>
