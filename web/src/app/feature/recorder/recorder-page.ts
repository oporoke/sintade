...
/**
 * Recorder setup (Day 27): choose what to share, whether to include system audio, and which
 * microphone. Starting, pausing and stopping arrive with the control bar (Day 29); device
 * check and countdown with Day 28.
 */
@Component({
...
export class RecorderPage {
  ...
  async onMicChange(event: Event): Promise<void> {
    ...
    try {
      const stream = await this.sources.openMic(deviceId);
