...
### In Progress
- M3 — Capture engine (Days 21–32, weeks 5–7): in progress. Framework-free `capture` package and `SourceManager` (Day 21) `AudioMixer` with level meters (Day 22) `ChunkRecorder` (Day 23) pause/resume + auto-stop (Day 24) the OPFS/IndexedDB `ChunkStore` (Day 25) and crash recovery (Day 26) done; recorder UI next (Day 27).
