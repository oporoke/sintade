...
> [!IMPORTANT]
> **Repository status: in development (Day 17 of the [daily build plan](docs/plan/daily-build-plan.md), M2 — Identity; M1 Foundation complete).** The Cargo workspace (`crates/kernel`, `crates/platform`, `crates/identity`, `bin/api`, `bin/worker`) exists and builds; local dependencies (Postgres, MinIO, Mailpit) run via `compose.yml` + `just deps-up`; the API serves `/healthz` and `/readyz` on `:8080` with RFC 9457 problem+json error responses; `platform` provides `ObjectStore`, a `SKIP LOCKED` job queue with a dead-letter queue, a transactional outbox relay, `Mailer`, and a Postgres-backed `RateLimiter`; an Angular shell (`web/`) exists with a `/debug` capability-matrix page, verified cross-browser in CI; GitHub Actions CI is green and builds/pushes Docker images for api/worker/web on merge to `main` (no staging VPS exists yet — see `docs/plan/PROGRESS.md`). `crates/identity`'s MVP session lifecycle is hardened: login/signup rate limits (`429`), CSRF double-submit on `/auth/refresh` and `/auth/logout`, and the security headers from §11 on every response — the Angular signup/login/verify/reset screens and silent refresh start Day 17. This README is derived entirely from the *Sintade — Product & Architecture Document* (the design document). Every feature, endpoint, table, variable and procedure below is **planned**, not implemented, unless explicitly marked otherwise. Items that the design document does not settle are marked `TODO: Verify`.
>
...
**Model:** role-based access control (RBAC) scoped per workspace, combined with resource-level checks (ownership, share-link policy). Roles are a Postgres enum `member_role`. Authorization entry point (planned): `TenancyService::authorize(claims, Permission, WorkspaceId)` and `ctx.require(Permission::…, &resource)` inside services.
...
| API docs | utoipa (OpenAPI) | `TODO: Verify` | Spec generation; frontend DTOs |
...
### Identity — 🟢 MVP complete, hardened (Angular UI next, Day 17)
...
### In Progress
- M2 — Identity (Days 11–20, weeks 3–4): in progress — full MVP session lifecycle plus hardening done (Days 11–16): domain types, register, login, refresh rotation + reuse detection, logout, `GET /me`, verify-email, forgot/reset-password, rate limits, CSRF double-submit, security headers; Angular signup/login/verify/reset screens and silent refresh next (Day 17).
